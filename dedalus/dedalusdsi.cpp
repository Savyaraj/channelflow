...
void dedalusDSI::chooseMu(continuationParameter mu) {
    cPar_ = mu;
    switch (mu) {
...
continuationParameter dedalusDSI::s2cPar(string muname) {
...
string dedalusDSI::cPar2s(continuationParameter cPar) {
...
void f(const FlowField& u, int N, Real dt, FlowField& f_u, const DNSFlags& flags_, ostream& os) {
    ...
    // f_u = fields[0];
    FlowField u_temp = u;
    PyObject* dns = init_dedalus();
    advance_dedalus(u_temp, N, dns);
    f_u = u_temp;
    return;
}

// return f^{N dt}(u) = time-(N dt) DNS integration of u
void f(const FlowField& u, Real T, FlowField& f_u, const DNSFlags& flags_, ostream& os) {
    os << "f(u, N, dt, f_u, flags, dt) : " << flush;
    DNSFlags flags(flags_);
    flags.logstream = &os;
    // vector<FlowField> fields(2);
...
Real GMRESHookstep_vector(FlowField& u, Real& T, FieldSymmetry& sigma, PoincareCondition* hpoincare,
    ...
    dedalusDSI Gx(dnsflags, sigma, hpoincare, dt, Tsearch, searchflags.xrelative, searchflags.zrelative, Tnormalize,
             Unormalize, u, os);
...
void advance_dedalus(FlowField& u, Real T, PyObject* de){
    ...
    // double* u_c = reinterpret_cast<double*>(PyArray_DATA(u_out));
